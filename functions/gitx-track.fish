function gitx-track --description 'Unignore and stage files into ~/.gitx/profiles/<profile>/repo'
    if test (count $argv) -lt 2
        echo "Usage: gitx-track [--dry-run] <profile> <file> [file ...]" >&2
        return 1
    end
    ...
    if not test -d "$repo"
        echo "gitx-track: profile repo not found: $repo" >&2
        return 1
    end
    ...
    for raw in $args[2..-1]
        ...
        if test $status -ne 0
            ...
            set -l blob_hash (command $hash_cmd)
    ...
    if test $dry_run -eq 1
        echo "Dry Run Plan: gitx-track $profile"
        echo
        ...
        echo "Summary: targets="(count $dry_targets)", exclude_entries="(count $dry_exclude)", track_paths="(count $dry_track)", commands="(count $dry_cmds)
    ...
    echo "Track Result: gitx-track $profile"
    echo
    ...
    echo "Summary: tracked=$tracked_count, staged=$staged_count, skipped="(count $run_skipped)", exclude_entries_added="(count $run_exclude)", commands="(count $run_cmds)
    echo
    __gitx_print_section "Next step" "gitx $profile commit -m \"message\""
end
